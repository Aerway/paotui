...
这里别装得太自信，下面是现在真实能做到和做不到的边界：

- 文件工具会把路径解析到工作目录内，避免正常情况下越界；但“检查路径”和“实际打开文件”之间仍有极端竞态情况，别把它当成高安全级别的沙箱。
- shell 的白名单是防呆，不是防坏。允许 `python` 这类命令，本质上就等于把闸门打开；`allow_all: true` 更是如此。它也只是不支持管道、重定向和变量等 shell 语法，并不替你判断命令本身安不安全。
- `web_fetch` 做了协议、内网地址和重定向的基本 SSRF 拦截，但 DNS 重绑定在校验与实际连接之间仍可能有竞态。
- HTTP 服务没有鉴权。默认只该绑在 `127.0.0.1`，不要为了图省事把它直接裸露到公网。

一句话：这是给自己用的小工具，别拿去裸奔公网。
